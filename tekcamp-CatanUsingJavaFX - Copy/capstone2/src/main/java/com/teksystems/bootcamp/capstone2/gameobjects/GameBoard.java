...
public class GameBoard extends Group {
   ...
   private final List<HexCordinate> vertices;
   private final List<HexEdge> edges;
...
// TODO  make randomize, and check token number
         children.add(new ImageViewWithAnImage().getImageViewWithAnImage(IMG_CATAN_GAMEBOARD_BACKGROUND, 800,800));
    ...
    public void initializeHex(int row, int column, int windowHeight, String imgURL, int tokenNumber, String terrain, GameBoard gameBoard) {
        {
            ...
            hex.setHexVertices(new HexVertices().getAllVertices(x,y+50,A6th,gameBoard));
            hex.setEdges(new HexEdges().getAllEdges(x,y+50,A6th,gameBoard));
            hex.setCenterPoint( new HexCordinate(x+(A12th),y+(A12th+50),gameBoard));
            gameBoard.getTokeNumbers().add(new TokenNumber(windowHeight,hex.getCenterPoint(),tokenNumber+".png"));


        }
    }
    public List<HexCordinate> getVertices() {
        return vertices;
    }
